...
const filteredProjects = computed(() => {
  if (!store.dictionary?.projects || !store.dictionary?.user) return []

  const currentUserId = store.dictionary.user.id
  return store.dictionary.projects.filter(project =>
     project.users?.some(user => user.user_id === currentUserId))
})
...
<template>
  <div class="q-py-lg">
    <div class="q-gutter-y-md column">
      ...
      <q-file  square dense bg-color="white" color="blue-7" v-model="file">
        <template v-slot:prepend>
        ...
        <template v-if="!file" v-slot:default>
          <div class="text-grey-7 absolute-left q-mt-sm">Прикрепить файлы</div>
        </template>
      </q-file>
      <div>
